...
export const MultiSelect = ({ field, options, setFieldValue }: any) => {
  ...
  return (
    <Popover>
      <PopoverTrigger className="flex justify-between mt-1 border border-gray px-3 py-2 rounded-lg">
        {field.value.length > 0 ? field.value.join(", ") : "Select"}
        <ChevronDown />
      </PopoverTrigger>
      <PopoverContent className="w-[311px] p-2 bg-white shadow-md rounded-lg">
