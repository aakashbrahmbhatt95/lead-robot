...
const IncludeSegment = ({ values }: any) => {
  return (
    <div className="py-5 px-3 mt-8 border-[1px] border-gray-300">
      <div className="flex items-center gap-2 border-b-[1px] border-gray-300 pb-3">
        <p>
          <span className="font-bold">Include</span> if contacts match
        </p>
        <Field as="select" name="includeCondition">
          {({ field }: any) => (
            <Select
              value={field.value}
      ...
      <FieldArray name="includeConditions">
        {({ remove, push }) => (
          <>
            {values.includeConditions.map(
              (includeCondition: any, index: any) => (
                <div key={index}>
                  {index > 0 && (
                    <p className="pt-3 font-bold">
                      {values?.includeCondition === "all" ? "AND" : "OR"}
                    </p>
                  )}
                  ...
                  <div className="flex items-center py-3 gap-2 border-b-[1px] border-gray-300 pb-3">
                    ...
                    <Field
                      as="select"
                      name={`includeConditions[${index}].field`}
                    >
                      {({ field }: any) => (
                        <Select
                          value={field.value}
                    ...
                    <Field
                      as="select"
                      name={`includeConditions[${index}].operator`}
                    >
                      {({ field }: any) => (
                        <Select
                          value={field.value}
                    ...
                    <Field
                      as={Input}
                      name={`includeConditions[${index}].value`}
                      className="w-[250px]"
                    />
                  </div>
                </div>
